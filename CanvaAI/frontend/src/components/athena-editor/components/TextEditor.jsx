...
// Context providers
import { EditorProvider, useEditorContext, useDocumentState, useFormattingState, useUIState, useEditorFeatures } from '../contexts/EditorContent.jsx';
import { ImageProvider, useImageContext, useImageUpload, useImageInsertion, useMediaManagement } from '../contexts/ImageContext.jsx';
...
const TextEditorContent = () => {
  ...
  // Destructure state from contexts (only for non-conflicting values)
  const { isAISidebarOpen, showReferencesPanel, showTemplateSidebar, showExportDialog, exportFormat, exportOptions, documentTitle, lastSaved, zoom, saveStatus, documentStats } = editorState;
  
  // Destructure actions from contexts
  const { setSaveStatus, setLastSaved, setDocumentStats } = editorActions;
  ...
  // Other state variables from previous implementation
  ...
  // Create a ref for the editor instance
  ...
  const editor = useEditor({
    extensions: [
      StarterKit.configure({
        ...
        link: true,
        underline: true,
        listItem: false, // Disable default listItem since we're using custom ones
        bulletList: false, // Disable default bulletList
        orderedList: false, // Disable default orderedList
        blockquote: true, // Enable blockquote from StarterKit
      }),
    ...
    content: `
      ...
      <ul class="bullet-list">
        <li class="list-item">Rich text formatting (bold, italic, underline, etc.)</li>
        <li class="list-item">Multiple heading levels with proper hierarchy</li>
        <li class="list-item">Smart lists and blockquotes</li>
        <li class="list-item">Code blocks with syntax highlighting</li>
        <li class="list-item">Resizable tables and images</li>
        <li class="list-item">Page breaks and print layout</li>
        <li class="list-item">AI-powered content generation and enhancement</li>
      </ul>
      <ol class="ordered-list">
        <li class="list-item">First step in your writing journey</li>
        <li class="list-item">Second step with more advanced features</li>
        <li class="list-item">Final step to master the editor</li>
      </ol>
      <div class="task-list">
        <div class="task-item">
          <label>
          ...
          <div>Complete the tutorial</div>
        </div>
        <div class="task-item">
          <label>
            <input type="checkbox" checked />
          </label>
          <div>Try the AI features</div>
        </div>
        <div class="task-item">
          <label>
            <input type="checkbox" />
          </label>
          <div>Export your first document</div>
        </div>
      </div>
      <blockquote class="blockquote" style="border-left: 4px solid #3b82f6; padding-left: 20px; margin: 24px 0; color: #6b7280; font-style: italic;">
    ...
    onUpdate: ({ editor }) => {
      ...
      // Debug: Log editor content structure
      console.log('Editor content updated:', {
        html: editor.getHTML(),
        text: editor.getText(),
        hasBulletList: editor.isActive('bulletList'),
        hasOrderedList: editor.isActive('orderedList'),
        hasTaskList: editor.isActive('taskList')
      });
    ...
    editorProps: {
      ...
      handleKeyDown: (view, event) => {
        ...
        if (event.key === '/' && (event.ctrlKey || event.metaKey)) {
          ...
          editorActions.updateEditorFeatures({ isAISidebarOpen: true });
          return true;
        }
  ...
  const insertImage = (src, alt = '', width = 400, height = 300, options = {}) => {
    ...
    try {
      ...
      editorActions.updateDocumentStats(prev => ({
        ...prev,
  ...
  const updateDocumentStats = useCallback((editor) => {
    ...
    editorActions.updateDocumentStats({
      paragraphs,
      images,
      tables,
      pages: pageCount
  ...
  const insertPageNumber = () => {
    if (editor) {
      ...
      // Insert page number with proper formatting
      editor.chain().focus().insertContent(
  ...
  const goToPage = (pageNumber) => {
    ...
    try {
      // Find page markers in the document
      const pageMarkers = [];
      editor.state.doc.descendants((node, pos) => {
        if (node.type.name === 'horizontalRule') {
          pageMarkers.push(pos);
        }
      });
      
      // Calculate target position (pageNumber - 1 because first page is before any breaks)
      const targetPageIndex = pageNumber - 1;
      
      if (targetPageIndex >= 0 && targetPageIndex < pageMarkers.length + 1) {
        let targetPos;
        if (targetPageIndex === 0) {
          // First page - go to start of document
          targetPos = 0;
        } else {
          // Subsequent pages - go to after the page break
          targetPos = pageMarkers[targetPageIndex - 1] + 1;
        }
  ...
  const handleAutoSave = () => {
    ...
    editorActions.setLastSaved(new Date());
    editorActions.setSaveStatus('saved');
  ...
  const handleExport = async () => {
    ...
    if (!editorContent || editorContent === '<p></p>' || editorContent.trim() === '<p></p>') {
      ...
      editorActions.updateEditorFeatures({ showExportDialog: false });
      return;
    }
    
    const options = {
      filename: `${documentTitle || 'document'}.${editorState.exportFormat}`,
      includePageNumbers: editorState.exportOptions.includePageNumbers,
      includeHeader: editorState.exportOptions.includeHeader,
      includeFooter: editorState.exportOptions.includeFooter,
      title: documentTitle || 'My Document'
    };

    try {
      switch (editorState.exportFormat) {
        case 'pdf':
        ...
        case 'html':
          ...
          break;
        default:
          toast.error(`Unsupported export format: ${editorState.exportFormat}`);
      }
    } catch (error) {
  ...
  const handleAIGenerate = async (content) => {
    ...
    editorActions.updateEditorFeatures({ isAISidebarOpen: false });
  };
  ...
  const handleZoomChange = (newZoom) => {
    editorActions.updateUIState({ zoom: newZoom });
  };

  const handleTemplateSelect = (template) => {
    if (editor) {
      editor.commands.setContent(template.content);
      editorActions.setDocumentTitle(template.name);
      toast.success(`Template "${template.name}" applied!`);
    }
  };

  const openExportDialog = () => {
    editorActions.updateEditorFeatures({ showExportDialog: true });
  };

  // Document Management Functions
  const handleRenameDocument = () => {
    if (isRenaming) {
      editorActions.setDocumentTitle(tempTitle);
      setIsRenaming(false);
  ...
  const handleDuplicateDocument = () => {
    ...
    editorActions.setDocumentTitle(newTitle);
  ...
  const handleDeleteDocument = () => {
    if (window.confirm('Are you sure you want to delete this document? This action cannot be undone.')) {
      ...
      editor?.commands.clearContent();
      editorActions.setDocumentTitle('Untitled Document');
    }
  };
  ...
  const handleRestoreVersion = (versionId) => {
    ...
    if (version && editor) {
      ...
      editorActions.setDocumentTitle(version.title);
  ...
  // FIXED: Enhanced toggle functions for lists and blockquote
  const toggleBulletList = () => {
    ...
    try {
      console.log('Current editor state before toggle:', {
        isActiveBullet: editor.isActive('bulletList'),
        isActiveOrdered: editor.isActive('orderedList'),
        isActiveTask: editor.isActive('taskList'),
      ...
      if (isActive) {
        ...
        console.log('Bullet list disabled');
      } else {
        ...
        setTimeout(() => {
          if (editor.isActive('bulletList')) {
            ...
            console.log('Applied automatic indentation to bullet list');
          }
        }, 50);
        
        toast.success('Bullet list enabled with indentation');
        console.log('Bullet list enabled');
      }
      
      // Log the state after the change
      setTimeout(() => {
        console.log('Editor state after toggle:', {
          isActiveBullet: editor.isActive('bulletList'),
          isActiveOrdered: editor.isActive('orderedList'),
          isActiveTask: editor.isActive('taskList')
        });
  ...
  const toggleOrderedList = () => {
    ...
    try {
      console.log('Current editor state before ordered list toggle:', {
        isActiveBullet: editor.isActive('bulletList'),
        isActiveOrdered: editor.isActive('orderedList'),
        isActiveTask: editor.isActive('taskList'),
        selection: editor.state.selection
      ...
      if (isActive) {
        ...
        console.log('Numbered list disabled');
      } else {
        ...
        setTimeout(() => {
          if (editor.isActive('orderedList')) {
            // Apply small indent to make list items visually distinct
            editor.chain().focus().updateAttributes('listItem', { indent: 1 }).run();
            console.log('Applied automatic indentation to ordered list');
          }
        }, 50);
        
        toast.success('Numbered list enabled with indentation');
        console.log('Numbered list enabled');
      }
      
      // Log the state after the change
      setTimeout(() => {
        console.log('Editor state after ordered list toggle:', {
          isActiveBullet: editor.isActive('bulletList'),
          isActiveOrdered: editor.isActive('orderedList'),
          isActiveTask: editor.isActive('taskList')
        });
    ...
    try {
      ...
      setTimeout(() => {
        if (editor.isActive('taskList')) {
          ...
          editor.chain().focus().updateAttributes('listItem', { indent: 1 }).run();
          console.log('Applied automatic indentation to task list');
        }
      }, 50);
  ...
  return (
    <div className="h-screen flex flex-col bg-background">
      ...
      <header className="flex items-center justify-between px-3 py-2 border-b border-border bg-background">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            <Input
              value={documentTitle}
              onChange={(e) => editorActions.setDocumentTitle(e.target.value)}
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            onClick={() => editorActions.updateEditorFeatures({ showExportDialog: true })}
            size="sm"
      ...
      <EditorToolbar 
        ...
        setShowReferencesPanel={(show) => editorActions.updateEditorFeatures({ showReferencesPanel: show })}
        setIsAISidebarOpen={(open) => editorActions.updateEditorFeatures({ isAISidebarOpen: open })}
        isAISidebarOpen={editorState.isAISidebarOpen}
      ...
      <div className="flex-1 flex overflow-hidden">
            ...
            >
              <div className="p-16">
                ...
                {columnLayout.count > 1 ? (
                  ...
                  >
                    <div style={{ transform: `scale(${(zoom || 100) / 100})`, transformOrigin: 'top left' }}>
                      <EditorContent 
                        editor={editor} 
        ...
        {/* AI Sidebar */}
        <AISidebar
          isOpen={editorState.isAISidebarOpen}
          onClose={() => editorActions.updateEditorFeatures({ isAISidebarOpen: false })}
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <Button 
              ...
              size="sm" 
              onClick={() => {
                const pageInput = prompt('Go to page number (1-${documentStats.pages || 1}):');
                if (pageInput) {
                  const pageNumber = parseInt(pageInput);
                  if (!isNaN(pageNumber) && pageNumber > 0 && pageNumber <= (documentStats.pages || 1)) {
                    goToPage(pageNumber);
                  } else {
            ...
            >
              Page {documentStats.pages || 1}
            </Button>
            <span>of {documentStats.pages || 1}</span>
          </div>
      ...
      {/* Export Dialog */}
      <Dialog open={editorState.showExportDialog} onOpenChange={(open) => editorActions.updateEditorFeatures({ showExportDialog: open })}>
        <DialogContent className="max-w-md bg-white" aria-describedby="export-dialog-description">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label>Format</Label>
              <Select value={editorState.exportFormat} onValueChange={(value) => editorActions.updateEditorFeatures({ exportFormat: value })}>
                <SelectTrigger>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                <div className="flex items-center space-x-2">
                  <Switch
                    id="pageNumbers"
                    checked={editorState.exportOptions.includePageNumbers}
                    onCheckedChange={(checked) => 
                      editorActions.updateExportOptions({ includePageNumbers: checked })
                    }
                  />
                ...
                <div className="flex items-center space-x-2">
                  <Switch
                    id="header"
                    checked={editorState.exportOptions.includeHeader}
                    onCheckedChange={(checked) => 
                      editorActions.updateExportOptions({ includeHeader: checked })
                    }
                  />
                ...
                <div className="flex items-center space-x-2">
                  <Switch
                    id="footer"
                    checked={editorState.exportOptions.includeFooter}
                    onCheckedChange={(checked) => 
                      editorActions.updateExportOptions({ includeFooter: checked })
                    }
                  />
                ...
                <div className="flex items-center space-x-2">
                  <Switch
                    id="comments"
                    checked={editorState.exportOptions.exportComments}
                    onCheckedChange={(checked) => 
                      editorActions.updateExportOptions({ exportComments: checked })
                    }
                  />
                ...
                <div className="flex items-center space-x-2">
                  <Switch
                    id="trackChanges"
                    checked={editorState.exportOptions.exportTrackChanges}
                    onCheckedChange={(checked) => 
                      editorActions.updateExportOptions({ exportTrackChanges: checked })
                    }
                  />
          ...
          <DialogFooter>
            <Button 
              variant="outline" 
              onClick={() => editorActions.updateEditorFeatures({ showExportDialog: false })}
              disabled={exportLoading[editorState.exportFormat]}
            >
              Cancel
            </Button>
            <Button 
              ...
              disabled={exportLoading[editorState.exportFormat]}
            >
              {exportLoading[editorState.exportFormat] ? (
                <>
